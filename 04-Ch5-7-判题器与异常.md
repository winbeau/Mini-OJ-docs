...
public class Demo3 {
    public static void main(String[] args) {
        ...
        Solution bad  = in -> { throw new RuntimeException("boom"); };   // Lambda 实现接口
...
编译运行:
```bash
javac -d build src/oj/core/*.java src/oj/judge/Solution.java src/oj/judge/SimpleJudge.java src/oj/Demo3.java
...
把 `SimpleJudge` 重构成 `Judge` 接口 + `AbstractJudge`(模板方法,`final judge` + 抽象 `compare`)+ `StandardJudge`/`SpecialJudge` 子类,加 `oj.exception.JudgeException`(代码见上文【新产物架构】)。多态调用 + 开闭原则;断言要加 `-ea`:
```bash
